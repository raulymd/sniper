...
""" 
	...
	Tring to automate the whole process
	NOTE: script must be executed in testbench folder

	Description: mcpat.py version of our system -> our mcpat and sniper logs
"""
...
def compute_dram_power(nread, nwrite, t, config): 
  ...
  #Raul
def compute_Flikker_power(total_page_cnt,approx_page_cnt,nread,nwrite,t,config,total_acc_latency):
	# assumtions
	page_size = 4096
	Flikker_mem_size = 128 * 1024 * 1024		# 128MB
	
	# refresh calculation
	
	I_full = 0.5				# 0.5mA (Vdd = 1.7v)
	F_regular = 15.625			# nomimnal ref rate: 0.064 s or 15.625 Hz
	F_low	  = dram_approx_ref		#dram_approx_ref		# approx ref rate: extracted by MemHierModel (for now 1 Hz)
	approx_var_size = approx_page_cnt * page_size;		
	#exact_ratio = float(Flikker_mem_size-approx_var_size)/Flikker_mem_size
	exact_ratio = float(total_page_cnt-approx_page_cnt)/total_page_cnt
	if(exact_ratio >= 3.1/4):
	  I_PASR = 0.5		# -> totally complete
	elif(exact_ratio > 1.0/2):
	  I_PASR = 0.47		# -> 3/4
	elif(exact_ratio > 1.0/4):
	  I_PASR = 0.44		# -> 1/2	
	elif(exact_ratio > 1.0/8):
	  I_PASR = 0.38		# -> 1/4
	elif(exact_ratio > 1.0/16):
	  I_PASR = 0.35		# -> 1/8
	else:
	  I_PASR = 0.33	  	# -> 1/16
	power_self_refresh = ((I_full - I_PASR)*(float(F_low)/F_regular) + I_PASR) * 1.7		# Vdd = 1.7v
	...
	#pdb.set_trace()		#Raul 
	ncycles = t * DRAM_CLOCK * 1e6
	naccesscycles = total_acc_latency * DRAM_CLOCK * 1e6
	percharge_time = t - total_acc_latency
	read_per = float(nread)/(nread+nwrite) * 100
	write_per = float(nwrite)/(nread+nwrite) * 100 
	'''
	print '============================================================================'
	print 'Percentage of time that all banks on the DRAM are in a precharged state: ', 100-100*naccesscycles/ncycles
	print 'The average time between ACT commands to this DRAM: ', 1e9*percharge_time/(nread+nwrite), 'ns'
	print 'The percentage of clock cycles which are outputting read data from the DRAM: ',   read_per*naccesscycles/ncycles
	print 'The percentage of clock cycles which are inputting write data to the DRAM: ',  write_per*naccesscycles/ncycles
	print 'CKE values are all 0, DDR SDRAM Output Power per individual DQ -> 5nW'
	sys.exit(0)
	'''
	# asdf
	#power_chip_dyn = input('Enter total power from sumarry (in W): ')		
	#power_chip_dyn = 0.179		# it is 179mW for test/api (constant for each application -> IO considered)
	if(benchmark == "binarization"):
		power_chip_dyn = 	0.1525	# binarization
	elif(benchmark == "kmeans"):
		power_chip_dyn = 	0.1276	# kmeans
	elif(benchmark == "sobel"):
		power_chip_dyn = 	0.2720	# sobel
	elif(benchmark == "jpeg"):
		power_chip_dyn = 	0.1041	# jpeg
	else:
	...
	write_dc = nwrite / sockets / (ncycles or 1)
	power_chip_dyn  = read_dc * DRAM_POWER_READ + write_dc * DRAM_POWER_WRITE
	return (power_chip_act * 0.05, power_self_refresh * 0.95) 		# considering the 0.05 and 0.95 chip usage ratio
	'''
	power_socket_dyn  = power_chip_dyn * chips_per_dimm * dimms_per_socket
	power_socket_stat = power_self_refresh * chips_per_dimm * dimms_per_socket
	
	#print chips_per_dimm,dimms_per_socket,sockets
	#print '====== Calculating dram power for test/api ======'	
	return (power_socket_dyn * sockets * 0.05, power_socket_stat * sockets * 0.95)	# ratio of operation considered


def dram_power(results, config): 
  return compute_Flikker_power(
	sum(results['dram.overall_addr_cnter']), 
	sum(results['dram.approx_addr_cnter']),
	sum(results['dram.reads']),
	sum(results['dram.writes']),
	results['global.time']*1e-15, 
	config,
	sum(results['dram.total-access-latency'])*1e-15
	)
...
def mcpat_run(inputfile, outputfile):
  ...
  assert os.system("../../mcpat/Raul/mcpat -print_level 5 -opt_for_clk 0 -infile %s > %s" % (inputfile, outputfile))==0
...
def main(setnum,jobid, resultsdir, outputfile, powertype = 'dynamic', config = None, no_graph = False, partial = None, print_stack = True, return_data = False):
  ...
  #Raul
  #print "power.xml made!" 
  #print stats['dram.reads']
  #print int(stats['dram.writes'][0])
  ...
  if powertype == 'area':	
    ...
    for core, (res, total, other, scale) in results.items():
      plot_data[core] = {}
      total_core = 0.; total_cache = 0.; total_dram=0.
      for name, value in res:
        if print_stack:
          energy, energy_scale = sniper_lib.scale_sci(float(value) * seconds)
          print '  %-12s    %6.6f W   %6.6f %sJ    %6.2f%%' % (name, float(value), energy, energy_scale, 100 * float(value) / total)
        if name.startswith('core'):
          total_core += float(value)
        elif name in ('icache', 'dcache', 'l2', 'l3', 'nuca'):
          total_cache += float(value)
      ...
      if print_stack:
        ...
        print '  %-12s    %6.6f W   %6.6f %sJ    %6.2f%%' % ('core', float(total_core), energy, energy_scale, 100 * float(total_core) / total)
        energy, energy_scale = sniper_lib.scale_sci(float(total_cache) * seconds)
        print '  %-12s    %6.6f W   %6.6f %sJ    %6.2f%%' % ('cache', float(total_cache), energy, energy_scale, 100 * float(total_cache) / total)
        energy, energy_scale = sniper_lib.scale_sci(float(total_dram) * seconds)
        print '  %-12s    %6.6f W   %6.6f %sJ    %6.2f%%' % ('dram', float(total_dram), energy, energy_scale, 100 * float(total_dram) / total)
        energy, energy_scale = sniper_lib.scale_sci(float(total) * seconds)
        print '  %-12s    %6.6f W   %6.6f %sJ    %6.2f%%' % ('total', float(total), energy, energy_scale, 100 * float(total) / total)
...
def power_stack(power_dat, powertype = 'total', nocollapse = False):
  ...
  return buildstack.merge_items({ 0: data }, all_items, nocollapse = True)


def edit_XML(statsobj, stats, cfg): 
  ncores = int(cfg['general/total_cores'])
  technology_node = 45 #int(sniper_config.get_config_default(cfg, 'power/technology_node', 45))	#65
    ...
    #for j in range(ncores):
      ...
      if template[i][1]:
        ...
        if len(template[i][1]) == 1:
          ...
          elif template[i][1][0]=="approx_add_instructions":
              if approx_alu_enable:
                template[i][0] = template[i][0] % stats['magic.approx_adds'][0]
              else:
                template[i][0] = template[i][0] % (0)
          ...
          #Raul
          elif template[i][1][0]=="dcache.approxVdd":
            template[i][0] = template[i][0] % L1_approx_vdd
          elif template[i][1][0]=="dcache.approx_read_accesses":
            template[i][0] = template[i][0] % int(stats['L1-D.approx-loads'][core])
          elif template[i][1][0]=="dcache.approx_write_accesses":
            template[i][0] = template[i][0] % int(stats['L1-D.approx-stores'][core])
          elif template[i][1][0]=="dcache.approx_read_misses":
            template[i][0] = template[i][0] % int(stats['L1-D.approx-load-misses'][core])
          elif template[i][1][0]=="dcache.approx_write_misses":
            template[i][0] = template[i][0] % int(stats['L1-D.approx-store-misses'][core])
          elif template[i][1][0]=="dcache.approx_ratio":
            template[i][0] = template[i][0] % int(stats['L1-D.approx-ratio'][core])  
			
          #Raul_2
          elif template[i][1][0]=="dcache.approxVdd_2":
            template[i][0] = template[i][0] % L1_approx_vdd_2
          elif template[i][1][0]=="dcache.approx_read_accesses_2":
            template[i][0] = template[i][0] % int(stats['L1-D.approx-loads-2'][core])
          elif template[i][1][0]=="dcache.approx_write_accesses_2":
            template[i][0] = template[i][0] % int(stats['L1-D.approx-stores-2'][core])
          elif template[i][1][0]=="dcache.approx_read_misses_2":
            template[i][0] = template[i][0] % int(stats['L1-D.approx-load-misses-2'][core])
          elif template[i][1][0]=="dcache.approx_write_misses_2":
            template[i][0] = template[i][0] % int(stats['L1-D.approx-store-misses-2'][core])
          elif template[i][1][0]=="dcache.approx_ratio_2":
            template[i][0] = template[i][0] % int(stats['L1-D.approx-ratio-2'][core])  			
          ...
          #Raul
          elif template[i][1][0]=="L2.approxVdd":
            template[i][0] = template[i][0] % L2_approx_vdd
          elif template[i][1][0]=="L2.approx_read_accesses":
            template[i][0] = template[i][0] % sum([ stats['L2.approx-loads'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.approx_write_accesses":
            template[i][0] = template[i][0] % sum([ stats['L2.approx-stores'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.approx_read_misses":
            template[i][0] = template[i][0] % sum([ stats['L2.approx-load-misses'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.approx_write_misses":
            template[i][0] = template[i][0] % sum([ stats['L2.approx-store-misses'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.approx_ratio":
            tmp = [ stats['L2.approx-ratio'][c] for c in cores_l2s ]
            template[i][0] = template[i][0] % (sum(tmp)/float(len(tmp)))  #average
			
          #Raul_2
          elif template[i][1][0]=="L2.approxVdd_2":
            template[i][0] = template[i][0] % L2_approx_vdd_2
          elif template[i][1][0]=="L2.approx_read_accesses_2":
            template[i][0] = template[i][0] % sum([ stats['L2.approx-loads-2'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.approx_write_accesses_2":
            template[i][0] = template[i][0] % sum([ stats['L2.approx-stores-2'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.approx_read_misses_2":
            template[i][0] = template[i][0] % sum([ stats['L2.approx-load-misses-2'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.approx_write_misses_2":
            template[i][0] = template[i][0] % sum([ stats['L2.approx-store-misses-2'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.approx_ratio_2":
            tmp = [ stats['L2.approx-ratio-2'][c] for c in cores_l2s ]
            template[i][0] = template[i][0] % (sum(tmp)/float(len(tmp)))  #average			
...
#Important stats to be extracted
    ...
    template.append(["\t\t\t<stat name=\"appadd_instructions\" value=\"%i\"/>",["approx_add_instructions","stat",iCount]])        #stats--
...
#       template.append(["\t\t\t\t<param name=\"buffer_sizes\" value=\"%i, 16, %i, %i\"/>",["icache_buffer","comb"]]) #mcpat will crash for some different sizes of 2nd parameter
    ...
    template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>",""])
    #Raul
    template.append(["\t\t\t\t<stat name=\"approxVdd\" value=\"%f\"/>",["dcache.approxVdd","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_read_accesses\" value=\"%i\"/>",["dcache.approx_read_accesses","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_write_accesses\" value=\"%i\"/>",["dcache.approx_write_accesses","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_read_misses\" value=\"%i\"/>",["dcache.approx_read_misses","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_write_misses\" value=\"%i\"/>",["dcache.approx_write_misses","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_ratio\" value=\"%i\"/>",["dcache.approx_ratio","stat",iCount]])
    #Raul_2
    template.append(["\t\t\t\t<stat name=\"approxVdd_2\" value=\"%f\"/>",["dcache.approxVdd_2","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_read_accesses_2\" value=\"%i\"/>",["dcache.approx_read_accesses_2","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_write_accesses_2\" value=\"%i\"/>",["dcache.approx_write_accesses_2","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_read_misses_2\" value=\"%i\"/>",["dcache.approx_read_misses_2","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_write_misses_2\" value=\"%i\"/>",["dcache.approx_write_misses_2","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_ratio_2\" value=\"%i\"/>",["dcache.approx_ratio_2","stat",iCount]])	
...
#------------------------------------------------------------------------------------------------------------------------
  for iCount in range(num_l2s):
    ...
    #Raul
    template.append(["\t\t\t\t<stat name=\"approxVdd\" value=\"%f\"/>",["L2.approxVdd","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_read_accesses\" value=\"%i\"/>",["L2.approx_read_accesses","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_write_accesses\" value=\"%i\"/>",["L2.approx_write_accesses","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_read_misses\" value=\"%i\"/>",["L2.approx_read_misses","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_write_misses\" value=\"%i\"/>",["L2.approx_write_misses","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_ratio\" value=\"%i\"/>",["L2.approx_ratio","stat",iCount]])
    #Raul_2
    template.append(["\t\t\t\t<stat name=\"approxVdd_2\" value=\"%f\"/>",["L2.approxVdd_2","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_read_accesses_2\" value=\"%i\"/>",["L2.approx_read_accesses_2","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_write_accesses_2\" value=\"%i\"/>",["L2.approx_write_accesses_2","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_read_misses_2\" value=\"%i\"/>",["L2.approx_read_misses_2","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_write_misses_2\" value=\"%i\"/>",["L2.approx_write_misses_2","stat",iCount]])
    template.append(["\t\t\t\t<stat name=\"approx_ratio_2\" value=\"%i\"/>",["L2.approx_ratio_2","stat",iCount]])
...
L2_approx_vdd = 0
dram_approx_ref = 1
approx_alu_enable = False
benchmark = "NONE"
def run(bench,a,b,c,d,prin):
  #print "inside run..."
  global L1_approx_vdd,L2_approx_vdd,dram_approx_ref
  global benchmark
  L1_approx_vdd = a
  L2_approx_vdd = b
  dram_approx_ref = c
  approx_alu_enable = d
  benchmark = bench
  retval = main(setnum = 0,jobid = 0, resultsdir = '.', powertype = 'total', config = None, outputfile = 'power', no_graph = True, print_stack = False, partial = None, return_data = True)
  #print retval['power_data'][0]['dcache']   #L1 data
  #print retval['power_data'][0]['l2']       #L2 data
  #print retval['power_data'][0]['dram']     #dram data
  #pdb.set_trace()
  if(prin):
    print ('--------------------------')
    print ("\t\tEnergy")
    for i in range(retval['ncores']):
      print 'Core %d:' %(i)
      print '  Int ALU: \t%f' %(retval['power_data'][i]['core-int'])
      print '  Mul ALU: \t%f' %(retval['power_data'][i]['core-alu'])
      print '  L1 dcache: \t%f' %(retval['power_data'][i]['dcache'])   #L1 data
      print '  L2 cache: \t%f'  %(retval['power_data'][i]['l2'])
    if retval['power_data'][0]['l3']  != 0:        # system has L3 cache
      print 'L3 cache: \t%f'  %(retval['power_data'][0]['l3'])
    print 'DRAM: \t\t%f'  %(retval['power_data'][0]['dram'])
#        print '  %-12s    %6.2f W   %6.2f %sJ    %6.2f%%' % (name, float(value), energy, energy_scale, 100 * float(value) / total)

  return (retval['power_data'][0]['dcache'],retval['power_data'][0]['l2'],retval['power_data'][0]['dram'] )
	
L1_approx_vdd_2 = 0
L2_approx_vdd_2 = 0
dram_approx_ref_2 = 1
def run_2(a,b,c,d,e,f,prin):		# only for sobel
  #print "inside run..."
  global L1_approx_vdd,L2_approx_vdd,dram_approx_ref
  global L1_approx_vdd_2,L2_approx_vdd_2,dram_approx_ref_2
  global benchmark 
  benchmark = "sobel"
  L1_approx_vdd = a
  L2_approx_vdd = b  
  ...
  L2_approx_vdd_2 = e
  dram_approx_ref_2 = f
  retval = main(setnum = 1,jobid = 0, resultsdir = '.', powertype = 'total', config = None, outputfile = 'power', no_graph = True, print_stack = prin, partial = None, return_data = True)
  dram_retval = retval['power_data'][0]['dram']
  if (c != f):
    _,_,tmp = run("sobel",1,1,f,False)
    dram_retval = (dram_retval+tmp)/2
    if(prin == True):
        print "WARNING: check out return value for dram power"
        print (retval['power_data'][0]['dcache'],retval['power_data'][0]['l2'],dram_retval )	
  #print retval['power_data'][0]['dcache']   #L1 data
  #print retval['power_data'][0]['l2']       #L2 data
  #print retval['power_data'][0]['dram']     #dram data
  return (retval['power_data'][0]['dcache'],retval['power_data'][0]['l2'],dram_retval )	
...
# asdf
  # normal power
  #assert os.system("mv	sim_exact.stats.sqlite3	sim.stats.sqlite3")==0	
  #val = run("binarization",1,1,15.625,False)					#normal power
  #val = run("kmeans",1,1,15.625,False)					#normal power
  #val = run("jpeg",1,1,15.625,False)					#normal power
  #assert os.system("mv	sim.stats.sqlite3		sim_exact.stats.sqlite3")==0
  #print val

  # main is always commented	
  ...
  #assert os.system("mv	sim_oneset.stats.sqlite3	sim.stats.sqlite3")==0	
  # ----- ESM - 45nm ------ #
  #binarization
  #val = run("binarization"	,0.6,0.7,0.05,False)		#20 dB
  #val = run("binarization"	,0.7,0.7,0.05,False)		#28 dB
  #val = run("binarization"	,0.7,0.9,0.05,False)		#35 dB
  #kmeans 
  #val = run("kmeans"		,0.6,0.6,0.05,False)		#20 dB
  #val = run("kmeans"		,0.7,0.7,0.05,False)		#28 dB
  #val = run("kmeans"		,0.7,0.9,0.05,False)		#35 dB
  #jpeg
  #val = run("jpeg"			,0.6,0.6,0.05,False)		#20 dB
  #val = run("jpeg"			,0.6,0.7,0.05,False)		#28 dB
  #val = run("jpeg"			,0.7,0.7,0.05,False)		#35 dB
  #sobel
  #val = run("sobel"		,0.6,0.7,0.05,False)		#20 dB
  #val = run("sobel"		,0.7,0.7,0.05,False)		#28 dB
  #val = run("sobel"		,0.7,0.9,0.05,False)		#35 dB
  
  # ----- ESM - 65nm ------ #
  #binarization
  #val = run("binarization"	,0.8,0.9,0.05,False)		#20 dB
  #val = run("binarization"	,0.9,1.0,0.05,False)		#28 dB
  #\val = run("binarization"	,0.9,1.0,0.05,False)		#35 dB
  #kmeans 
  #val = run("kmeans"		,0.8,0.9,0.05,False)		#20 dB
  #val = run("kmeans"		,0.9,1.0,0.05,False)		#28 dB
  #val = run("kmeans"		,0.9,1.0,0.05,False)		#35 dB
  #jpeg
  #val = run("jpeg"			,0.7,0.8,0.05,False)		#20 dB
  #val = run("jpeg"			,0.8,0.9,0.05,False)		#28 dB
  #val = run("jpeg"			,0.8,1.0,0.05,False)		#35 dB
  #sobel
  #val = run("sobel"		,0.8,0.9,0.05,False)		#20 dB
  #val = run("sobel"		,0.9,1.0,0.05,False)		#28 dB
  #val = run("sobel"		,1,1,0.05,False)			#35 dB
  
  # ----- PFM - 45nm ------ #
  #binarization
  #val = run("binarization"	,0.7,0.7,0.05,False)		#20 dB
  #val = run("binarization"	,0.7,0.8,0.05,False)		#28 dB
  #val = run("binarization"	,0.7,0.9,0.05,False)		#35 dB
  #kmeans 
  #val = run("kmeans"		,0.6,0.7,0.05,False)		#20 dB
  #val = run("kmeans"		,0.7,0.7,0.05,False)		#28 dB
  #val = run("kmeans"		,0.7,0.9,0.05,False)		#35 dB
  #jpeg
  #val = run("jpeg"			,0.6,0.6,0.05,False)		#20 dB
  ...
  #val = run("jpeg"			,0.7,0.7,0.05,False)		#35 dB
  #sobel
  #val = run("sobel"		,0.7,0.7,0.05,False)		#20 dB
  #val = run("sobel"		,0.7,0.8,0.05,False)		#28 dB
  #val = run("sobel"		,0.7,0.9,0.05,False)		#35 dB
  ...
  #kmeans 
  #val = run("kmeans"		,0.8,0.9,0.05,False)		#20 dB
  #val = run("kmeans"		,0.9,0.9,0.05,False)		#28 dB
  #val = run("kmeans"		,0.9,1.0,0.05,False)		#35 dB
  #jpeg
  #val = run("jpeg"			,0.7,0.9,0.05,False)		#20 dB
  #val = run("jpeg"			,0.8,0.9,0.05,False)		#28 dB
  #val = run("jpeg"			,0.8,1.0,0.05,False)		#35 dB
  #sobel
  #val = run("sobel"		,0.8,0.9,0.05,False)		#20 dB
  #val = run("sobel"		,0.9,1.0,0.05,False)		#28 dB
  #val = run("sobel"		,0.9,1,0.05,False)			#35 dB
  #assert os.system("mv	sim.stats.sqlite3		sim_oneset.stats.sqlite3")==0
  #print val
  
  #normal mode		
  """
  assert os.system("mv	sim_exact.stats.sqlite3	sim.stats.sqlite3")==0	
  val = run_2(1.0,1.0,15.625,1.0,1.0,15.625,False)			#normal power		(L1,L2,dr,L1_2,L2_2,dr_2,True)		
  print val
  assert os.system("mv	sim.stats.sqlite3		sim_exact.stats.sqlite3")==0
  """
  #one set
  """
  assert os.system("mv	sim_oneset.stats.sqlite3	sim.stats.sqlite3")==0	
  val = run_2(1.0,0.9,0.05,1.0,0.9,0.05,False)	
  print val
  assert os.system("mv	sim.stats.sqlite3		sim_oneset.stats.sqlite3")==0  
  """
  #two set -> only sobel up to now
  #val = run_2(1.0,1.0,15.625,1.0,1.0,15.625,False)	
  #print val, sum(val)  
  #val = run_2(1.0,0.8,0.05,1.0,0.8,0.05,False)	
  #print val, sum(val)
  #val = run_2(0.9,0.8,0.05,1.0,0.9,0.05,False)	
  #print val, sum(val)
  #val = run_2(0.9,0.9,0.05,0.8,0.8,0.05,False)	
  #print val, sum(val)
  #val = run_2(1.0,0.8,0.05,0.9,0.8,0.05,False)	
  #print val, sum(val)
  #val = run_2(1.0,0.9,0.05,0.8,0.7,0.05,False)	
  #print val , sum(val) 
  #val = run_2(0.9,0.8,0.05,0.9,0.8,0.05,False)	
  #print val , sum(val)   
  #val = run_2(1.0,1.0,0.05,0.9,0.8,0.05,False)	
  #print val , sum(val)     
  #assert os.system("mv	sim.stats.sqlite3		sim_twoset.stats.sqlite3")==0   
  """
  """
  val = run("sobel",1,1,15.625,False,True)              #normal power, this is the entry
  #assert os.system("mv	power_non.xml	power.xml")==0	
  #assert os.system("mv	power.xml	power_non.xml")==0	